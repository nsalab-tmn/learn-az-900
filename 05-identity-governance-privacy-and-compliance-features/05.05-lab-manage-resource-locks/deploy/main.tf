...
resource "azurerm_role_definition" "allow-locks" {
  name        = "allow-locks-role-${var.instance_id}"
  scope       = azurerm_resource_group.learn.id
  description = "This is a custom role created via Terraform to allow locks"
...
resource "azurerm_role_assignment" "allow-locks" {
  scope                = azurerm_resource_group.learn.id
  role_definition_name = azurerm_role_definition.allow-locks.name
  principal_id         = azuread_user.learn.object_id

  depends_on = [
    azurerm_role_definition.allow-locks
  ]
}
