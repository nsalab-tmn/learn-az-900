...
resource "azurerm_resource_group_policy_assignment" "allowed-serverfarms-os-assignment" {
  name                 = "serverfarms-os-assignment-${var.tp_name}-${var.instance_id}"
  resource_group_id    = azurerm_resource_group.learn.id
  policy_definition_id = azurerm_policy_definition.allowed-serverfarms-os-definition.id
}

resource "azurerm_resource_group_policy_assignment" "allowed-serverfarms-sku-assignment" {
  name                 = "serverfarms-sku-assignment-${var.tp_name}-${var.instance_id}"
  resource_group_id    = azurerm_resource_group.learn.id
  policy_definition_id = azurerm_policy_definition.allowed-serverfarms-sku-definition.id
}
